...
use dynamic_island::view::{
    build_dynamic_island_page_state, create_root, render_dynamic_island_page,
};
...
let page = build_dynamic_island_page_state(&snapshot, true, false);
let plan = root.render(render_dynamic_island_page(&page));
```

运行时路径和这个 page 入口复用同一个 `IslandWidget` 组件树；page 只负责 `RuntimeSnapshot -> page state -> widget`，尺寸和动画参数仍由 renderer 在最后一跳注入。
