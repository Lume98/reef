...
fn main() {
    ...
    for (i, p) in plan.primitives.iter().enumerate() {
        match p {
            ...
            DrawPrimitive::Rect { frame, color, .. } => {
                println!("[{}] Rect       @ ({:.0},{:.0}) {:.0}x{:.0}  #{:02x}{:02x}{:02x}",
                    i, frame.x, frame.y, frame.width, frame.height, color.r, color.g, color.b);
            }
