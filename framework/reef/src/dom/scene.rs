...
impl SceneNode {
    pub fn new(id: u64, ty: &str, props: PropsMap) -> Self {
        let flex = props
            .get("flex")
            .and_then(|v| {
                if let crate::vnode::PropValue::F64(f) = v {
                    Some(*f)
                } else {
                    None
                }
    ...
    pub fn prop_str(&self, key: &str) -> Option<String> {
        self.props.get(key).and_then(|v| {
            if let crate::vnode::PropValue::String(s) = v {
                Some(s.clone())
            } else {
                None
            }
    ...
    pub fn prop_color(&self, key: &str) -> Option<Color> {
        self.props.get(key).and_then(|v| {
            if let crate::vnode::PropValue::Color(c) = v {
                Some(*c)
            } else {
                None
            }
    ...
    pub fn prop_i32(&self, key: &str) -> Option<i32> {
        self.props.get(key).and_then(|v| {
            if let crate::vnode::PropValue::I32(i) = v {
                Some(*i)
            } else {
                None
            }
    ...
    pub fn prop_bool(&self, key: &str) -> Option<bool> {
        self.props.get(key).and_then(|v| {
            if let crate::vnode::PropValue::Bool(b) = v {
                Some(*b)
            } else {
                None
            }
...
mod tests {
    ...
    fn scene_node_count() {
        ...
        parent
            .children
            .push(SceneNode::new(2, "label", PropsMap::new()));
        parent
            .children
            .push(SceneNode::new(3, "label", PropsMap::new()));
