use crate::hooks::{begin_fiber, end_fiber, FiberId};
use crate::reconciler::arena::FiberArena;
use crate::reconciler::fiber::{EffectTag, ElementTypeRef};
use crate::reconciler::host_config::{HostConfig, HostInstanceId};
use crate::reconciler::reconcile::reconcile_children;
...
impl WorkLoop {
    ...
    fn commit_root(&mut self, arena: &mut FiberArena, root_id: FiberId, host: &mut dyn HostConfig) {
                ...
                match &element_type {
                    ElementTypeRef::Native(_) => {
                        let (ty, props, return_to) = {
                            ...
                            (
                                ElementType::Native(
                                    if let ElementTypeRef::Native(n) = &f.element_type {
                                        n
                                    } else {
                                        ""
                                    },
...
mod tests {
    use super::*;
    use crate::reconciler::arena::FiberArena;
    use crate::reconciler::fiber::FiberNode;
    ...
    impl HostConfig for TestHost {
        fn create_instance(
            &mut self,
            _ty: &ElementType,
            _props: &crate::vnode::PropsMap,
        ) -> HostInstanceId {
            HostInstanceId(0)
        }
        fn append_child(&mut self, _parent: HostInstanceId, _child: HostInstanceId) {}
        fn update_instance(&mut self, _instance: HostInstanceId, _props: &crate::vnode::PropsMap) {}
    ...
    fn work_loop_reconciles_root() {
        ...
        wl.reconcile_root(
            &mut arena,
            root,
            vec![VNode::VElement(crate::vnode::VElement {
                ty: ElementType::Native("label"),
                props: crate::vnode::PropsMap::new(),
                children: vec![],
                key: None,
            })],
        );
