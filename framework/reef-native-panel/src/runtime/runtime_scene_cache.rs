...
pub(crate) fn build_panel_scene_for_state_bridge_with_input<S>(
    state: &S,
...
pub(crate) fn resolve_panel_scene_for_state_bridge_with_input<S>(
    state: &S,
    input: &NativePanelRuntimeInputDescriptor,
) -> Option<PanelScene>
where
    S: NativePanelRuntimeSceneStateBridge,
{
    ...
    cached_scene_for_key(state.runtime_scene_cache(), &cache_key).or_else(|| {
        state
            .runtime_scene_current_snapshot()
            .map(|snapshot| build_panel_scene_for_state_bridge_with_input(state, snapshot, input))
    })
}

pub(crate) fn resolve_panel_scene_for_state_bridge_and_snapshot_with_input<S>(
    state: &S,
    snapshot: &RuntimeSnapshot,
    input: &NativePanelRuntimeInputDescriptor,
) -> Option<PanelScene>
    ...
    S: NativePanelRuntimeSceneStateBridge,
{
    ...
    Some(build_panel_scene_for_state_bridge_with_input(
    ...
    S: NativePanelRuntimeSceneStateBridge,
{
    ...
    let scene = render_command_bundle
        ...
        .or_else(|| {
            resolve_panel_scene_for_state_bridge_and_snapshot_with_input(state, snapshot, input)
        })?;
...
{
    let scene = resolve_panel_scene_for_state_bridge_with_input(state, input)?;
    let runtime =
...
mod tests {
    use super::{
        apply_runtime_scene_bundle_to_host, build_panel_scene_for_state_bridge_with_input,
    ...
    impl NativePanelRuntime for TestRenderer {
        type Error = String;
    ...
    fn state_bridge_helpers_rebuild_from_current_snapshot_when_cache_misses() {
        ...
        let rebuilt_scene =
            build_panel_scene_for_state_bridge_with_input(&state, &snapshot, &input);
        let rebuilt_runtime =
            resolve_native_panel_runtime_render_state_for_state_bridge_with_input(&state, &input);
    ...
    fn mutable_state_bridge_resolve_and_cache_presentation_syncs_bundle_and_regions() {
        ...
        let scene = build_panel_scene_for_state_bridge_with_input(
            &state,
        ...
        PanelRuntimeSceneBundle {
            scene: PanelScene {
                ...
                settings_surface: build_settings_surface_scene(
                    crate::scene::resolve_settings_surface_projection(
                        &[crate::scene::fallback_panel_display_option()],
                        PanelSettingsState::default(),
                    ),
                    PanelSettingsState::default(),
                    "0.0.0",
                    &crate::updater_service::AppUpdateStatus::idle(),
                    crate::scene::PanelInteractionProfile::Standalone,
                ),
                cards: vec![],
                glow: None,
    ...
    fn test_render_command_bundle(status: &str, transitioning: bool) -> NativePanelRenderBundle {
        ...
        let layout = crate::state::resolve_panel_layout(crate::state::PanelLayoutInput {
            ...
            metrics: crate::state::PanelGeometryMetrics {
                compact_height: crate::state::DEFAULT_COMPACT_PILL_HEIGHT,
                compact_width: crate::state::DEFAULT_COMPACT_PILL_WIDTH,
                expanded_width: crate::state::DEFAULT_EXPANDED_PILL_WIDTH,
                panel_width: crate::state::DEFAULT_PANEL_CANVAS_WIDTH,
            },
            canvas_height: 180.0,
        ...
        let render_state =
            crate::state::resolve_panel_render_state(crate::state::PanelRenderStateInput {
