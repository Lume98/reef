...
use crate::state::{PanelAnimationTimeline, PANEL_ANIMATION_FRAME_MS};
...
mod tests {
            ...
            final_frame = scheduler
                .sample(
                    now + Duration::from_millis(
                        200 + step * crate::state::PANEL_ANIMATION_FRAME_MS,
                    ),
                )
                .expect("frame tick");
        }
    ...
    fn animation_scheduler_throttles_samples_between_frame_ticks() {
        ...
        assert!(scheduler
            .sample(now + Duration::from_millis(crate::state::PANEL_ANIMATION_FRAME_MS - 1))
            .is_none());

        let next = scheduler
            .sample(now + Duration::from_millis(crate::state::PANEL_ANIMATION_FRAME_MS))
            .expect("next frame at frame tick");

        assert_eq!(next.elapsed_ms, crate::state::PANEL_ANIMATION_FRAME_MS);
    }
    ...
    fn animation_scheduler_keeps_one_non_terminal_frame_after_delayed_wake() {
        ...
        assert_eq!(
            ...
            crate::state::PANEL_ANIMATION_FRAME_MS * 4
        );
            ...
            .div_ceil(crate::state::PANEL_ANIMATION_FRAME_MS)
        {
            ...
            final_frame = scheduler
                .sample(
                    now + Duration::from_millis(
                        first.total_ms + 250 + step * crate::state::PANEL_ANIMATION_FRAME_MS,
                    ),
                )
