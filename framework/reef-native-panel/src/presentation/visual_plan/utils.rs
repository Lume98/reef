use crate::state::{lerp, resolve_estimated_text_width, PanelRect};
use reef::core::color::Color;
use reef::draw::primitive::DrawPrimitive;

use crate::state::PanelChromeVisibilitySpec;
    ...
    match primitive {
        ...
        DrawPrimitive::Path { segments, .. } => segments
            ...
            .map(|segment| match segment {
                reef::draw::primitive::PathSegment::LineTo(point) => point.y,
    ...
    match primitive {
        ...
        DrawPrimitive::Path { segments, .. } => {
            for segment in segments {
                match segment {
                    reef::draw::primitive::PathSegment::LineTo(point) => point.y += translate_y,
                    reef::draw::primitive::PathSegment::CubicBezier {
                        control1,
                        control2,
                        end,
...
pub fn compact_digit_y(bar_height: f64) -> f64 {
    ((bar_height - crate::state::ACTIVE_COUNT_LABEL_HEIGHT) / 2.0).round() - 1.5
}
