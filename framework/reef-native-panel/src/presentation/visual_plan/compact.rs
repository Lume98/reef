...
use reef::draw::primitive::{DrawPlan, DrawPrimitive, PathSegment};
...
use reef::theme::{compact_bar as compact_theme, panel as theme};
...
pub fn resolve_native_panel_compact_bar_visual_plan(input: &NativePanelPaintInput) -> DrawPlan {
    if input.display_mode == NativePanelVisualDisplayMode::Hidden || !input.window_state.visible {
        return DrawPlan {
            hidden: true,
            viewport: reef::core::geometry::Size {
                width: 0.0,
                height: 0.0,
            },
            primitives: Vec::new(),
...
pub(super) fn push_compact_headline_primitive(
    ...
    let headline_width = crate::state::resolve_estimated_text_width(&headline_text, 13.0)
...
fn compact_shoulder_path_segments(
    ...
    let bottom = frame.y;
    let control_y = frame.y + frame.height * crate::state::COMPACT_SHOULDER_CURVE_FACTOR;
    match side {
        NativePanelVisualShoulderSide::Left => {
            let left = frame.x + frame.width * (1.0 - scale_x);
            let right = frame.x + frame.width;
            let control_x =
                right - frame.width * (1.0 - crate::state::COMPACT_SHOULDER_CURVE_FACTOR) * scale_x;
            vec![
        ...
        NativePanelVisualShoulderSide::Right => {
            ...
            let control_x =
                frame.x + frame.width * crate::state::COMPACT_SHOULDER_CURVE_FACTOR * scale_x;
            vec![
                PathSegment::LineTo(draw_point(PanelPoint { x: right, y: top })),
                PathSegment::LineTo(draw_point(PanelPoint { x: left, y: top })),
