use crate::scene::{PanelScene, SceneCard, SceneHitTarget};
use crate::state::{
...
use crate::state::HoverTransition;
...
fn push_mascot_bubble_hover_region(
    ...
    let has_bubble = scene.compact_bar.completion_count > 0
        || scene.mascot_pose == crate::scene::SceneMascotPose::MessageBubble;
    if !has_bubble {
        return;
    }
...
fn push_settings_row_hit_target_regions(
    ...
    index: usize,
    row: Option<&crate::scene::SettingsRowScene>,
    ...
    if let Some(row) = row {
        use crate::presentation::card_visual_spec::{
