...
use crate::presentation::card_visual_spec::{card_visual_settings_row_layout, CardVisualRowSpec};
use crate::scene::{build_panel_scene, PanelSceneBuildInput};
use crate::state::{
...
fn pointer_test_scene() -> crate::scene::PanelScene {
...
fn pointer_regions_use_default_edge_action_frames_without_platform_input() {
    ...
    let pill = crate::state::absolute_rect(layout.panel_frame, layout.pill_frame);
    let action_layout = crate::state::resolve_compact_action_button_layout(pill);
    assert_eq!(
        settings_frame,
        PanelRect {
...
fn pointer_regions_use_settings_gap_for_debug_mode_trigger() {
    ...
    scene.mascot_pose = crate::scene::SceneMascotPose::Hidden;
...
fn settings_pointer_regions_match_visible_rows_without_settings_folder_action() {
    ...
    let settings_card_frame = PanelRect {
        x: cards.x,
        y: cards.y
            - (crate::state::resolve_settings_surface_card_height(6) - cards.height).max(0.0),
        width: cards.width,
        height: crate::state::resolve_settings_surface_card_height(6),
    };
...
fn pointer_regions_include_mascot_bubble_hover_overhang() {
    ...
    scene.mascot_pose = crate::scene::SceneMascotPose::Complete;
...
fn host_window_descriptor_projects_animation_and_window_state() {
    let descriptor = NativePanelHostWindowDescriptor {
        ...
        timeline: Some(NativePanelTimelineDescriptor {
            animation: crate::state::PanelAnimationDescriptor {
                kind: crate::state::PanelAnimationKind::Open,
                canvas_height: 140.0,
...
fn host_window_descriptor_helpers_update_shared_fields() {
    let animation = crate::state::PanelAnimationDescriptor {
        kind: crate::state::PanelAnimationKind::Open,
        canvas_height: 140.0,
        visible_height: 120.0,
        width_progress: 0.5,
...
fn timeline_descriptor_for_animation_derives_card_direction() {
    let mut animation = crate::state::PanelAnimationDescriptor {
        kind: crate::state::PanelAnimationKind::Open,
        canvas_height: 140.0,
        visible_height: 120.0,
        width_progress: 0.5,
    ...
    animation.kind = crate::state::PanelAnimationKind::SurfaceSwitch;
    assert!(native_panel_timeline_descriptor_for_animation(animation).cards_entering);

    animation.kind = crate::state::PanelAnimationKind::Close;
    assert!(!native_panel_timeline_descriptor_for_animation(animation).cards_entering);
}
