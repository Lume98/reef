use crate::{
    scene::{SceneCard, SettingsRowScene},
    state::{
...
impl From<reef::theme::Rgb> for CardVisualColorSpec {
    fn from(value: reef::theme::Rgb) -> Self {
        Self {
            r: value.r,
            g: value.g,
            b: value.b,
...
fn card_visual_spec(
    ...
    CardVisualSpec {
        ...
        animation: CardVisualAnimationSpec {
            ...
            reveal_translate_y_from: crate::state::PANEL_CARD_REVEAL_Y,
            content_reveal_delay_progress: crate::state::PANEL_CARD_CONTENT_REVEAL_DELAY_PROGRESS,
            content_early_exit_progress: crate::state::PANEL_CARD_CONTENT_EARLY_EXIT_PROGRESS,
        },
    }
}
...
pub fn card_visual_content_reveal_frame(phase: f64) -> CardVisualContentRevealFrameSpec {
    ...
    let delay = crate::state::PANEL_CARD_CONTENT_REVEAL_DELAY_PROGRESS;
...
pub fn card_visual_content_transition_frame(
    ...
    let phase = phase.clamp(0.0, 1.0);
    let exit = crate::state::PANEL_CARD_CONTENT_EARLY_EXIT_PROGRESS;
    CardVisualContentRevealFrameSpec {
...
pub fn card_visual_stack_reveal_frame(
    ...
    let delay_ms = card_index as f64 * crate::state::PANEL_CARD_REVEAL_STAGGER_MS as f64;
    CardVisualStackRevealFrameSpec {
        progress,
        elapsed_ms,
        card_phase: ((elapsed_ms - delay_ms) / crate::state::PANEL_CARD_REVEAL_MS as f64)
            .clamp(0.0, 1.0),
    }
}
...
pub fn card_visual_staggered_phase(
    ...
    let duration_ms = if entering {
        ...
        crate::state::PANEL_CARD_EXIT_MS
    };
    let stagger_ms = if entering {
        crate::state::PANEL_CARD_REVEAL_STAGGER_MS
    } else {
        crate::state::PANEL_CARD_EXIT_STAGGER_MS
    };
...
pub fn card_visual_content_visibility_phase(phase: f64, entering: bool) -> f64 {
    ...
    } else if phase <= crate::state::PANEL_CARD_CONTENT_EARLY_EXIT_PROGRESS {
        let exit = crate::state::PANEL_CARD_CONTENT_EARLY_EXIT_PROGRESS;
