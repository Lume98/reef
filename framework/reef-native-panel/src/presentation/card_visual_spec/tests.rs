...
use super::*;
use crate::scene::{SceneBadge, SceneCard};
...
fn card_spec_exposes_mac_card_animation_constants() {
    ...
    assert_eq!(
        ...
        crate::state::PANEL_CARD_REVEAL_Y
    );
    assert_eq!(
        spec.animation.content_reveal_delay_progress,
        crate::state::PANEL_CARD_CONTENT_REVEAL_DELAY_PROGRESS
    );
    assert_eq!(
        spec.animation.content_early_exit_progress,
        crate::state::PANEL_CARD_CONTENT_EARLY_EXIT_PROGRESS
    );
}
...
fn card_spec_exposes_shared_content_exit_animation_frame() {
    ...
    let closing_hidden = card_visual_content_transition_frame(
        crate::state::PANEL_CARD_CONTENT_EARLY_EXIT_PROGRESS - 0.01,
        true,
    );
