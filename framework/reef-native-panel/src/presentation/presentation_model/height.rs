...
use crate::{
    scene::{resolve_scene_card_height_input, PanelScene, SceneCard},
    state::{
...
pub fn resolve_native_panel_presentation_metrics(
    ...
    use crate::state::EXPANDED_MAX_BODY_HEIGHT;
...
pub fn estimated_scene_content_height(scene: &PanelScene) -> f64 {
    estimated_scene_content_height_for_card_width(
        scene,
        crate::state::resolve_expanded_cards_width(
...
pub fn estimated_scene_card_height_for_card_width(card: &SceneCard, card_width: f64) -> f64 {
    match resolve_scene_card_height_input(card) {
        crate::scene::SceneCardHeightInput::Settings { row_count } => {
            resolve_settings_surface_card_height(row_count)
        }
        crate::scene::SceneCardHeightInput::PendingPermission(pending) => {
            pending_permission_card_height(pending, card_width)
        }
        crate::scene::SceneCardHeightInput::PendingQuestion(pending) => {
            pending_question_card_height(pending, card_width)
        }
        crate::scene::SceneCardHeightInput::PromptAssist(session) => {
            prompt_assist_card_height(session, card_width)
        }
        crate::scene::SceneCardHeightInput::Session(session) => {
            session_card_height(session, card_width)
        }
        crate::scene::SceneCardHeightInput::StatusItem(item) => {
            status_queue_card_height(item, card_width)
        }
        crate::scene::SceneCardHeightInput::Empty => EMPTY_CARD_HEIGHT,
    }
}
...
fn pending_permission_card_height(pending: &PendingPermissionView, card_width: f64) -> f64 {
    ...
    pending_like_card_height(
        &body,
        crate::state::PENDING_PERMISSION_CARD_MIN_HEIGHT,
        crate::state::PENDING_PERMISSION_CARD_MAX_HEIGHT,
        card_width,
    )
}
...
fn pending_question_card_height(pending: &PendingQuestionView, card_width: f64) -> f64 {
    ...
    let min_height = if pending.options.is_empty() {
        ...
        PENDING_QUESTION_CARD_MIN_HEIGHT + crate::state::PENDING_QUESTION_CARD_OPTIONS_EXTRA_HEIGHT
    };
    pending_like_card_height(
        &body,
        min_height,
        crate::state::PENDING_QUESTION_CARD_FALLBACK_MAX_HEIGHT
...
fn prompt_assist_card_height(_session: &SessionSnapshotView, card_width: f64) -> f64 {
    pending_like_card_height(
        ...
        crate::state::PROMPT_ASSIST_CARD_MIN_HEIGHT,
        crate::state::PROMPT_ASSIST_CARD_MAX_HEIGHT,
        card_width,
    )
}
