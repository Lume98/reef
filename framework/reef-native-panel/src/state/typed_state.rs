...
// 兼容层：从旧类型转换
impl From<crate::state::PanelRenderLayerStyleInput> for PanelRenderLayerStyle<InputStage> {
    fn from(old: crate::state::PanelRenderLayerStyleInput) -> Self {
        Self::new_input(
            old.shell_visible,
...
// 兼容层：转换到旧类型
impl From<PanelRenderLayerStyle<ComputedStage>> for crate::state::PanelRenderLayerStyleState {
...
impl From<crate::state::SharedExpandedContentInput> for SharedExpandedContent<InputStage> {
    fn from(old: crate::state::SharedExpandedContentInput) -> Self {
        Self::new_input(
            old.enabled,
            old.shell_visible,
            old.height_progress,
...
impl From<SharedExpandedContent<ComputedStage>> for crate::state::SharedExpandedContentState {
...
mod tests {
    ...
    fn test_backward_compatibility() {
        // 旧类型 → 新类型
        let old_input = crate::state::SharedExpandedContentInput {
            enabled: true,
            shell_visible: true,
        ...
        // 新类型 → 旧类型
        let old_state: crate::state::SharedExpandedContentState = computed.into();
