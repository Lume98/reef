...
use crate::{
    ...
    state::{clamp_panel_rect_to_bounds, PanelPoint, PanelRect},
};
#[derive(Clone, Copy, Debug, PartialEq)]
pub(super) struct WindowsPanelPointerPollingSample {
    pub(super) point: PanelPoint,
}

use super::hit_region::{resolve_windows_panel_hit_test, WindowsPanelHitTest};

use super::dpi::{resolve_windows_dpi_scale_for_window, WindowsDpiScale, WindowsPhysicalRect};

#[cfg(all(windows, not(test)))]
use super::hit_region::WindowsPanelHitTest as WindowsPanelPlatformHitTest;

#[cfg(all(windows, not(test)))]
use super::layered_window::apply_windows_layered_window_initial_attributes;
#[cfg(all(windows, not(test)))]
use super::paint_backend::{
    windows_panel_composition_mode_for_preferred_painter, WINDOWS_PANEL_TRANSPARENT_COLOR_KEY,
};
use super::{
    paint_backend::WindowsPanelPaintPlan,
    window_shell::{WindowsPanelShellCommand, WindowsPanelShellPaintJob},
};

#[derive(Clone, Debug, Default)]
pub(super) struct WindowsPanelPlatformLoopState {
    ...
    pub(super) last_painted_job: Option<WindowsPanelShellPaintJob>,
    pub(super) last_paint_plan: Option<WindowsPanelPaintPlan>,
...
pub(super) struct WindowsPanelQueuedWindowMessage {
...
static WINDOWS_PANEL_WINDOW_MESSAGE_QUEUE: OnceLock<Mutex<Vec<WindowsPanelQueuedWindowMessage>>> =
    OnceLock::new();

static WINDOWS_PANEL_HIT_REGION_CACHE: OnceLock<
    Mutex<Vec<(isize, Vec<NativePanelPointerRegion>)>>,
> = OnceLock::new();

fn windows_panel_window_message_queue() -> &'static Mutex<Vec<WindowsPanelQueuedWindowMessage>> {
    WINDOWS_PANEL_WINDOW_MESSAGE_QUEUE.get_or_init(|| Mutex::new(Vec::new()))
}

fn windows_panel_hit_region_cache() -> &'static Mutex<Vec<(isize, Vec<NativePanelPointerRegion>)>> {
    WINDOWS_PANEL_HIT_REGION_CACHE.get_or_init(|| Mutex::new(Vec::new()))
}

pub(super) fn sync_windows_panel_hit_regions(
    ...
    let Ok(mut cache) = windows_panel_hit_region_cache().lock() else {
        return;
    };
...
pub(super) fn clear_windows_panel_hit_regions(raw_window_handle: Option<isize>) {
    let Some(raw_window_handle) = raw_window_handle else {
        return;
    };
    let Ok(mut cache) = windows_panel_hit_region_cache().lock() else {
...
pub(super) fn resolve_windows_panel_cached_hit_test(
    raw_window_handle: isize,
    point: PanelPoint,
) -> WindowsPanelHitTest {
    let Ok(cache) = windows_panel_hit_region_cache().lock() else {
        return WindowsPanelHitTest::Transparent;
    };
    ...
    resolve_windows_panel_hit_test(regions, point)
}

pub(crate) fn queue_windows_panel_window_message(hwnd: isize, message_id: u32, lparam: isize) {
    if let Ok(mut queue) = windows_panel_window_message_queue().lock() {
        queue.push(WindowsPanelQueuedWindowMessage {
            hwnd,
            message_id,
            lparam,
        });
...
pub(super) fn take_windows_panel_window_messages(
    raw_window_handle: Option<isize>,
) -> Vec<WindowsPanelQueuedWindowMessage> {
    let Some(raw_window_handle) = raw_window_handle else {
        return Vec::new();
    };
    let Ok(mut queue) = windows_panel_window_message_queue().lock() else {
        return Vec::new();
    };
...
#[cfg(test)]
pub(crate) fn clear_windows_panel_window_messages(raw_window_handle: Option<isize>) {
    let _ = take_windows_panel_window_messages(raw_window_handle);
}
...
extern "system" fn native_panel_window_proc(
    ...
    match msg {
        ...
        WM_NCHITTEST => {
            ...
            match resolve_windows_panel_cached_hit_test(hwnd as isize, point) {
                WindowsPanelPlatformHitTest::Client => HTCLIENT as isize,
                WindowsPanelPlatformHitTest::Transparent => HTTRANSPARENT as isize,
            }
        }
        super::window_shell::WINDOWS_WM_PAINT => unsafe {
            ...
            queue_windows_panel_window_message(hwnd as isize, msg, lparam);
            0
        },
        ...
        | super::window_shell::WINDOWS_WM_MOUSELEAVE => {
            if msg == super::window_shell::WINDOWS_WM_MOUSEMOVE {
                track_windows_panel_mouse_leave(hwnd);
            }
...
#[cfg(all(windows, not(test)))]
fn track_windows_panel_mouse_leave(hwnd: windows_sys::Win32::Foundation::HWND) {
...
impl NativePanelHostShellCommandBackend for WindowsPanelPlatformLoopState {
    ...
    fn destroy_shell_window(
        ...
        clear_windows_panel_hit_regions(raw_window_handle);
...
fn apply_windows_native_window_create(
    ...
    apply_windows_layered_window_initial_attributes(
        hwnd,
        windows_panel_composition_mode_for_preferred_painter(),
        WINDOWS_PANEL_TRANSPARENT_COLOR_KEY,
    )?;
...
#[derive(Clone, Copy, Debug, PartialEq)]
struct WindowsPanelSurfaceState {
    physical_rect: Option<WindowsPhysicalRect>,
    dpi_scale: WindowsDpiScale,
}
...
pub(super) fn ensure_windows_native_platform_loop_thread(
    ...
    crate::platform::windows::ensure_windows_native_platform_loop_thread(pump_runtime_once);
}

pub(super) fn platform_loop_thread_started() -> bool {
    crate::platform::windows::platform_loop_thread_started()
}

pub(super) fn wake_windows_native_platform_loop() {
    crate::platform::windows::wake_windows_native_platform_loop();
}

pub(super) fn schedule_windows_native_platform_loop_wake(delay_ms: u64) {
    crate::platform::windows::schedule_windows_native_platform_loop_wake(delay_ms);
}

#[cfg(all(windows, not(test)))]
pub(super) fn current_windows_panel_pointer_polling_sample(
    raw_window_handle: Option<isize>,
) -> Option<WindowsPanelPointerPollingSample> {
    use windows_sys::Win32::{
    ...
    Some(WindowsPanelPointerPollingSample {
...
pub(crate) fn windows_native_platform_loop_generations() -> Option<(u64, u64)> {
    crate::platform::windows::windows_native_platform_loop_generations()
}
...
pub(crate) fn wait_windows_native_platform_loop_processed_at_least(
    ...
    crate::platform::windows::wait_windows_native_platform_loop_processed_at_least(
        target_generation,
        timeout_ms,
    )
}
