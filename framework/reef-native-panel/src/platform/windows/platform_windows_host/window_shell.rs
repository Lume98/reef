use super::{draw_presenter::WindowsPanelDrawPresenter, host_window::WindowsPanelDrawFrame};
use crate::{
    runtime::facade::{
        descriptor::{
        ...
        presentation::{
            ...
            native_panel_visual_plan_input_from_presentation, NativePanelPaintInput,
...
pub(super) struct WindowsPanelWindowHandle {
    pub(super) hwnd: Option<isize>,
}

impl NativePanelPlatformWindowHandleAdapter for WindowsPanelWindowHandle {
...
pub(super) type WindowsPanelShellCommand = NativePanelHostShellCommand;

pub(super) type WindowsPanelShellPaintJob = NativePanelPaintInput;

pub(super) type WindowsPanelShellActionButtonPaintInput = NativePanelVisualActionButtonInput;

#[derive(Clone, Debug, PartialEq)]
pub(super) struct WindowsPanelShellDisplaySnapshot {
    pub(super) display_mode: NativePanelVisualDisplayMode,
    pub(super) visual_input: NativePanelPaintInput,
    pub(super) shared_visible: bool,
    pub(super) pointer_region_count: usize,
}

#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub(super) struct WindowsPanelShellPresentResult {
...
pub(super) struct WindowsPanelWindowShell {
    shell_state: NativePanelHostShellState,
    handle: WindowsPanelWindowHandle,
    last_frame: Option<WindowsPanelDrawFrame>,
    pending_paint_job: Option<WindowsPanelShellPaintJob>,
    last_painted_job: Option<WindowsPanelShellPaintJob>,
    paint_pass_count: usize,
    display_snapshot: Option<WindowsPanelShellDisplaySnapshot>,
...
impl WindowsPanelWindowShell {
    ...
    pub(super) fn display_snapshot(&self) -> Option<&WindowsPanelShellDisplaySnapshot> {
        self.display_snapshot.as_ref()
    }

    pub(super) fn pending_paint_job(&self) -> Option<&WindowsPanelShellPaintJob> {
        self.pending_paint_job.as_ref()
    }

    pub(super) fn last_painted_job(&self) -> Option<&WindowsPanelShellPaintJob> {
    ...
    pub(super) fn mascot_animation_needs_refresh(&self) -> bool {
        self.lightweight_refresh_visible()
            && self.display_snapshot.as_ref().is_some_and(|display| {
                display.visual_input.mascot_pose != crate::scene::SceneMascotPose::Hidden
            })
    }
    ...
    pub(super) fn refresh_mascot_animation(&mut self, elapsed_ms: u128) -> bool {
        ...
        if display.visual_input.mascot_pose == crate::scene::SceneMascotPose::Hidden {
            return false;
        }
    ...
    pub(super) fn take_pending_commands(&mut self) -> Vec<WindowsPanelShellCommand> {
    ...
    pub(super) fn paint_next_frame(&mut self) -> Option<WindowsPanelShellPaintJob> {
    ...
    pub(super) fn consume_presenter(
        ...
        let Some(frame) = presenter.take_redraw_frame() else {
            return WindowsPanelShellPresentResult::default();
        };
...
fn build_display_snapshot(frame: &WindowsPanelDrawFrame) -> WindowsPanelShellDisplaySnapshot {
    ...
    WindowsPanelShellDisplaySnapshot {
        display_mode,
...
fn build_paint_job(display: &WindowsPanelShellDisplaySnapshot) -> WindowsPanelShellPaintJob {
...
fn local_visual_frame(input: &NativePanelPaintInput, frame: PanelRect) -> PanelRect {
...
fn sync_presented_mascot_visual_state(
    display: &mut WindowsPanelShellDisplaySnapshot,
