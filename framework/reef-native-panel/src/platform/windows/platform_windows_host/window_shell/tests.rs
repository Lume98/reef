use super::{
    windows_client_hover_fallback_frames, WindowsPanelShellCommand, WindowsPanelWindowHandle,
    WindowsPanelWindowShell, WINDOWS_WM_LBUTTONDOWN, WINDOWS_WM_LBUTTONUP, WINDOWS_WM_MOUSELEAVE,
    WINDOWS_WM_MOUSEMOVE,
};
use crate::runtime::facade::shell::NativePanelPlatformWindowHandleAdapter;
use crate::{
    platform_windows_host::{
        draw_presenter::WindowsPanelDrawPresenter, host_window::WindowsPanelDrawFrame,
    },
    runtime::facade::{
        descriptor::{
...
fn presentation_with_mascot(
    pose: crate::scene::SceneMascotPose,
    shell_visible: bool,
    ...
    NativePanelPresentationModel {
        ...
        shell: NativePanelShellPresentation {
            surface: crate::state::ExpandedSurface::Default,
            frame: PanelRect {
                x: 0.0,
                y: 0.0,
                width: 320.0,
        ...
        compact_bar: NativePanelCompactBarPresentation {
            ...
            headline: crate::scene::SceneText {
...
fn shell_window_handle_helpers_roundtrip_handle_presence() {
    let mut handle = WindowsPanelWindowHandle::default();
...
fn shell_proxies_raw_window_handle_adapter() {
    let mut shell = WindowsPanelWindowShell::default();
...
fn shell_decodes_pointer_move_message_from_lparam() {
    let shell = WindowsPanelWindowShell::default();
...
fn shell_consumes_presenter_redraw_frame() {
    ...
    let mut shell = WindowsPanelWindowShell::default();
    presenter.present(WindowsPanelDrawFrame {
...
fn shell_emits_lifecycle_and_redraw_commands() {
    ...
    assert_eq!(
        ...
        vec![
            WindowsPanelShellCommand::Create,
            WindowsPanelShellCommand::Show,
            WindowsPanelShellCommand::SyncWindowState(state),
            WindowsPanelShellCommand::RequestRedraw,
            WindowsPanelShellCommand::Hide,
        ]
    );
}
...
fn shell_emits_mouse_passthrough_command_only_when_state_changes() {
    ...
    assert_eq!(
        shell.take_pending_commands(),
        vec![
            WindowsPanelShellCommand::SyncMouseEventPassthrough(true),
            WindowsPanelShellCommand::SyncMouseEventPassthrough(false),
        ]
    );
}
...
fn shell_builds_display_snapshot_from_presenter_frame() {
    ...
    presenter.present(WindowsPanelDrawFrame {
        ...
        presentation_model: Some(NativePanelPresentationModel {
            ...
            card_stack: NativePanelCardStackPresentation {
                ...
                surface: crate::state::ExpandedSurface::Status,
                cards: vec![crate::scene::SceneCard::Empty],
                content_height: 70.0,
                body_height: 70.0,
                visible: true,
            },
            mascot: NativePanelMascotPresentation {
                pose: crate::scene::SceneMascotPose::Complete,
                debug_mode_enabled: false,
            },
            glow: Some(NativePanelGlowPresentation {
                glow: crate::scene::SceneGlow {
                    style: crate::scene::SceneGlowStyle::Completion,
                    opacity: 0.8,
                },
            }),
            action_buttons: NativePanelActionButtonsPresentation {
    ...
    assert_eq!(
        ...
        crate::state::ExpandedSurface::Status
    );
    ...
    assert_eq!(
        ...
        crate::scene::SceneMascotPose::Complete
    );
...
fn shell_refreshes_idle_mascot_into_sleepy_after_shared_delay() {
    ...
    assert!(
        shell.refresh_mascot_animation(crate::state::MASCOT_IDLE_LONG_SECONDS as u128 * 1000 + 1)
    );

    let paint_job = shell.paint_next_frame().expect("paint job");
    assert_eq!(paint_job.mascot_pose, crate::scene::SceneMascotPose::Sleepy);
}
...
fn shell_plays_wake_angry_after_sleepy_when_mascot_becomes_active() {
    ...
    presenter.present(WindowsPanelDrawFrame {
        ...
        presentation_model: Some(presentation_with_mascot(
            crate::scene::SceneMascotPose::Running,
            false,
        )),
    });
    shell.consume_presenter(&mut presenter);
    ...
    assert_eq!(
        shell
            ...
            .visual_input
            .mascot_pose,
        crate::scene::SceneMascotPose::WakeAngry
    );
    let wake_started_at = crate::state::MASCOT_IDLE_LONG_SECONDS as u128 * 1000 + 1;
...
fn shell_keeps_sleepy_when_idle_compact_presenter_refreshes() {
    ...
    assert!(
        shell.refresh_mascot_animation(crate::state::MASCOT_IDLE_LONG_SECONDS as u128 * 1000 + 16)
    );
    assert_eq!(
        shell.paint_next_frame().expect("sleepy paint").mascot_pose,
        crate::scene::SceneMascotPose::Sleepy
    );
}
...
fn shell_preserves_mascot_elapsed_time_when_expanding_same_pose() {
    ...
    presenter.present(WindowsPanelDrawFrame {
        ...
        pointer_regions: Vec::new(),
        presentation_model: Some(presentation_with_mascot(
            crate::scene::SceneMascotPose::MessageBubble,
            false,
        )),
    });
    shell.consume_presenter(&mut presenter);
...
fn shell_hover_frames_use_windows_client_coordinates_and_stable_bubble_hover() {
    ...
    let frames = windows_client_hover_fallback_frames(
        &input,
        crate::runtime::facade::interaction::resolve_native_panel_hover_fallback_frames(&input),
    );
