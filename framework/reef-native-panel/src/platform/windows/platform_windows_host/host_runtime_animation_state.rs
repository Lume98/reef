...
use crate::{
    ...
    state::{panel_state_needs_status_queue_refresh, take_pending_status_reopen_after_transition},
};
...
impl WindowsPanelRuntime {
    ...
    pub(super) fn lightweight_refresh_plan_impl(
        &self,
    ) -> crate::state::NativePanelLightweightRefreshPlan {
        crate::state::resolve_native_panel_lightweight_refresh_plan(
            crate::state::NativePanelLightweightRefreshInput {
    ...
    pub(super) fn advance_animation_frame_at_impl(
        ...
        if let Some(request) = self.last_transition_request.take() {
            ...
            if request == NativePanelTransitionRequest::Open
                ...
                && self.panel_state.surface_mode == crate::state::ExpandedSurface::Status
            {
                let input = crate::runtime::facade::descriptor::NativePanelRuntimeInputDescriptor {
    ...
    pub(super) fn apply_animation_frame_impl(
        ...
        if !frame.continue_animating {
            ...
            self.next_animation_wake_at = None;
            if frame.descriptor.animation.kind == crate::state::PanelAnimationKind::Close {
                self.hover_close_in_progress = false;
                self.pending_close_presentation = None;
    ...
    pub(super) fn resolve_animation_target_impl(
        ...
        let start_height = self
            ...
            .unwrap_or(crate::state::COLLAPSED_PANEL_HEIGHT);
        let target_height = match request {
            NativePanelTransitionRequest::Close => crate::state::COLLAPSED_PANEL_HEIGHT,
    ...
    pub(super) fn resolved_expanded_target_height_impl(&self) -> f64 {
        let body_height = self
            ...
            .unwrap_or(0.0);
        (crate::state::DEFAULT_COMPACT_PILL_HEIGHT
            + crate::state::EXPANDED_CONTENT_TOP_GAP
            + body_height
            + crate::state::EXPANDED_CONTENT_BOTTOM_INSET)
            .max(crate::state::COLLAPSED_PANEL_HEIGHT)
    }
    ...
    pub(super) fn schedule_next_status_queue_refresh_wake_impl(&self) {
        if panel_state_needs_status_queue_refresh(&self.panel_state) {
            super::platform_loop::schedule_windows_native_platform_loop_wake(
                crate::state::STATUS_QUEUE_REFRESH_MS,
            );
            return;
        }
        ...
        if lightweight_refresh.active_count_marquee.refresh_allowed {
            super::platform_loop::schedule_windows_native_platform_loop_wake(
                crate::state::ACTIVE_COUNT_SCROLL_REFRESH_MS,
            );
            return;
        }
        if lightweight_refresh.mascot_animation.refresh_allowed {
            super::platform_loop::schedule_windows_native_platform_loop_wake(
                crate::state::MASCOT_ANIMATION_REFRESH_MS,
            );
            return;
        }
        super::platform_loop::schedule_windows_native_platform_loop_wake(
            crate::state::HOVER_POLL_MS,
        );
    }
}
