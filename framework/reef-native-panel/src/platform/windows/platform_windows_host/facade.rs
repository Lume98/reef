...
use super::runtime_entry::{
    drain_windows_panel_platform_events, spawn_platform_loops_internal, with_windows_panel_runtime,
};
use super::runtime_input::windows_runtime_input_descriptor_without_app;

static WINDOWS_PANEL_STANDALONE_EVENT_DISPATCH_LOOP_STARTED: AtomicBool = AtomicBool::new(false);
...
pub(crate) fn update_native_panel_snapshot_without_app(
    ...
    let sync =
        with_windows_panel_runtime(|runtime| runtime.sync_snapshot_bundle(snapshot, &input))?;
...
pub(crate) fn refresh_native_panel_from_last_snapshot_without_app() -> Result<(), String> {
    let input = windows_runtime_input_descriptor_without_app();
    with_windows_panel_runtime(|runtime| {
        runtime
...
pub(crate) fn reposition_native_panel_to_selected_display_without_app() -> Result<(), String> {
    let input = windows_runtime_input_descriptor_without_app();
    with_windows_panel_runtime(|runtime| runtime.reposition_to_selected_display_with_input(&input))
}

fn spawn_windows_panel_standalone_event_dispatch_loop() {
    use std::sync::atomic::Ordering;
    use std::time::Duration;

    if WINDOWS_PANEL_STANDALONE_EVENT_DISPATCH_LOOP_STARTED.swap(true, Ordering::SeqCst) {
        return;
    }
...
fn dispatch_queued_native_panel_platform_events_without_app() -> Result<(), String> {
    let events = drain_windows_panel_platform_events()?;
    if events.is_empty() {
        return Ok(());
    }
