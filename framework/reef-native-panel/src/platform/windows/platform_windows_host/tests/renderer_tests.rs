...
fn windows_renderer_caches_scene_and_resolves_shared_render_inputs() {
    ...
    let mut renderer = super::WindowsPanelRenderer::default();
    ...
    let expected_layout = crate::state::resolve_panel_layout(
        crate::state::PanelLayoutInput {
...
fn windows_host_shell_can_consume_presenter_frame() {
    let mut host = super::WindowsPanelHost::default();
    host.presenter.present(WindowsPanelDrawFrame {
...
fn windows_runtime_pointer_move_syncs_mouse_passthrough_state() {
    ...
    assert!(runtime
        ...
        .any(|command| matches!(
            command,
            super::window_shell::WindowsPanelShellCommand::SyncMouseEventPassthrough(false)
        )));
}
...
fn windows_runtime_pointer_leave_syncs_mouse_passthrough_state() {
    ...
    assert!(runtime
        ...
        .any(|command| matches!(
            command,
            super::window_shell::WindowsPanelShellCommand::SyncMouseEventPassthrough(true)
        )));
}
...
fn windows_runtime_host_polling_interaction_updates_passthrough_state() {
    ...
    assert_eq!(
        ...
        vec![
            crate::runtime::facade::interaction::NativePanelHostBehaviorCommand::SetMouseEventPassthrough {
                ignores_mouse_events: false,
            }
        ]
    );
    ...
    let mut runtime = super::WindowsPanelRuntime {
        panel_state: PanelState {
            pointer_inside_since: Some(
                now - Duration::from_millis(crate::state::HOVER_DELAY_MS + 100),
            ),
            completion_badge_items: vec![CompletionBadgeItem {
                session_id: "session-1".to_string(),
                completed_at: Utc::now(),
...
fn windows_runtime_host_polling_interaction_resolves_hit_target_click() {
    ...
    assert!(!runtime
        ...
        .any(|command| matches!(
            command,
            super::window_shell::WindowsPanelShellCommand::SyncMouseEventPassthrough(_)
        )));
}
