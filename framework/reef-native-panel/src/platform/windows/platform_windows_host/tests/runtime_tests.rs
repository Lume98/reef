...
fn windows_runtime_and_host_satisfy_shared_native_traits() {
    ...
    assert_runtime::<super::WindowsPanelRuntime>();
    assert_host::<super::WindowsPanelHost>();
}
...
fn windows_runtime_emits_generic_completion_reminder_for_active_to_idle_without_message() {
    ...
    assert!(runtime.panel_state.status_queue.iter().any(|item| matches!(
        item.payload,
        crate::state::StatusQueuePayload::Completion(_)
    )));
}
...
fn windows_runtime_reopens_completion_when_message_arrives_after_expired_generic_card() {
    ...
    runtime.panel_state.status_queue = vec![crate::state::StatusQueueItem {
        ...
        is_removing: false,
        remove_after: None,
        payload: crate::state::StatusQueuePayload::Completion(previous_session),
    }];
...
fn windows_host_lifecycle_tracks_create_show_hide() {
    let mut host = super::WindowsPanelHost::default();

    assert_eq!(
        host.window.lifecycle,
        super::host_window::WindowsPanelWindowLifecycle::NotCreated
    );
    assert_eq!(
    ...
    assert_eq!(
        host.window.lifecycle,
        super::host_window::WindowsPanelWindowLifecycle::Created
    );
    assert_eq!(
        host.shell.lifecycle(),
        NativePanelHostShellLifecycle::Visible
...
fn windows_host_shell_commands_track_lifecycle_and_reposition() {
    ...
    assert!(commands.iter().any(|command| matches!(
        command,
        super::window_shell::WindowsPanelShellCommand::Create
    )));
    assert!(commands.iter().any(|command| matches!(
        command,
        super::window_shell::WindowsPanelShellCommand::Show
    )));
    assert!(commands.iter().any(|command| matches!(
        command,
        super::window_shell::WindowsPanelShellCommand::Hide
    )));
    assert!(commands.iter().any(|command| matches!(
        command,
        super::window_shell::WindowsPanelShellCommand::SyncWindowState(
...
fn windows_runtime_syncs_hover_expand_from_cached_regions() {
    ...
    runtime.panel_state.pointer_inside_since =
        Some(now - Duration::from_millis(crate::state::HOVER_DELAY_MS + 100));
...
fn windows_runtime_expanded_target_height_prefers_current_native_content_over_stale_shared_height()
{
    ...
    let expected_height = crate::state::DEFAULT_COMPACT_PILL_HEIGHT
        + crate::state::EXPANDED_CONTENT_TOP_GAP
        + 70.0
        + crate::state::EXPANDED_CONTENT_BOTTOM_INSET;
...
fn windows_runtime_expanded_target_height_prefers_latest_scene_over_stale_presentation_slot() {
    ...
    let stale_height = crate::state::DEFAULT_COMPACT_PILL_HEIGHT
        + crate::state::EXPANDED_CONTENT_TOP_GAP
        + 70.0
        + crate::state::EXPANDED_CONTENT_BOTTOM_INSET;
...
fn windows_runtime_expanded_target_height_uses_compact_width_when_scene_has_no_layout_bundle() {
    ...
    input.scene_input.settings.island_width_preset =
        crate::state::PanelIslandWidthPreset::Compact;
    ...
    let expected_body_height =
        crate::presentation::presentation::estimated_scene_content_height_for_card_width(
            scene, card_width,
        )
