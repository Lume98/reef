...
fn windows_animation_scheduler_finishes_close_transition_without_redraw_loop() {
    ...
    for step in 1..=first
        .total_ms
        .div_ceil(crate::state::PANEL_ANIMATION_FRAME_MS)
        + 1
    {
        let frame = runtime
            .advance_animation_frame_at(
                now + Duration::from_millis(
                    first.total_ms + step * crate::state::PANEL_ANIMATION_FRAME_MS,
                ),
            )
...
fn windows_runtime_refreshes_mascot_animation_paint_job_without_scene_sync() {
    ...
    runtime.mascot_animation_started_at =
        Some(now - Duration::from_millis(crate::state::MASCOT_ANIMATION_REFRESH_MS));
...
fn windows_animation_scheduler_preserves_status_cards_when_auto_status_close_skips_default_rebuild()
{
    ...
    let next_frame = runtime
        .advance_animation_frame_at(
            Instant::now()
                + Duration::from_millis(crate::state::PANEL_ANIMATION_FRAME_MS),
        )
...
fn windows_repeated_hover_cycles_keep_transition_stages_stable() {
    fn finish_animation(
        runtime: &mut super::WindowsPanelRuntime,
        start: Instant,
        total_ms: u64,
    ) {
        for step in 1..=total_ms.div_ceil(crate::state::PANEL_ANIMATION_FRAME_MS) + 2 {
            let Some(frame) = runtime
                .advance_animation_frame_at(
                    start
                        + Duration::from_millis(
                            step * crate::state::PANEL_ANIMATION_FRAME_MS,
                        ),
                )
                .expect("advance animation")
            else {
    ...
    for cycle in 0..5 {
        runtime.panel_state.pointer_inside_since =
            Some(now - Duration::from_millis(crate::state::HOVER_DELAY_MS + 1));
        assert_eq!(
            runtime
...
fn windows_runtime_replaces_stale_action_button_paint_before_status_close() {
    ...
    super::clear_windows_panel_window_messages(Some(hwnd));
    super::queue_windows_panel_window_message(hwnd, super::WINDOWS_WM_PAINT, 0);
    runtime.panel_state.status_queue[0].expires_at = Instant::now() - Duration::from_millis(1);
