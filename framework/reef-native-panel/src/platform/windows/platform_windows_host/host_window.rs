...
pub(super) enum WindowsPanelWindowLifecycle {
    #[default]
    NotCreated,
    Created,
}
...
pub(super) struct WindowsPanelHostWindow {
    pub(super) lifecycle: WindowsPanelWindowLifecycle,
...
#[derive(Clone, Debug)]
pub(crate) struct WindowsPanelDrawFrame {
...
impl WindowsPanelHostWindow {
    ...
    pub(super) fn take_pending_draw_frame(&mut self) -> Option<WindowsPanelDrawFrame> {
        if !self.pending_redraw {
            return None;
        }
        self.pending_redraw = false;
        self.presented_window_state
            .map(|window_state| WindowsPanelDrawFrame {
                window_state,
...
impl NativePanelComputedHostWindow for WindowsPanelHostWindow {
    ...
    fn set_host_window_created(&mut self) {
        self.lifecycle = WindowsPanelWindowLifecycle::Created;
    }
...
#[cfg(test)]
mod tests {
    use super::{windows_resolve_windows_panel_window_frame, WindowsPanelHostWindow};
    ...
    fn refresh_frame_uses_shared_host_window_descriptor_helper() {
        let mut host = WindowsPanelHostWindow {
    ...
    fn host_window_presents_pointer_regions_and_draw_invalidates_once() {
        let mut host = WindowsPanelHostWindow::default();
