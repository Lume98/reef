use super::{
    ...
    paint_backend::{resolve_windows_panel_paint_plan, WindowsPanelPaintPlan},
    window_shell::WindowsPanelShellPaintJob,
};
use crate::presentation::rendering::native_panel_submit_visual_plan;
use crate::state::{PanelPoint, PanelRect};
use reef::core::geometry::{Point, Rect};
...
use super::{
    ...
    dpi::WindowsDpiScale,
    paint_backend::{
        resolve_windows_panel_hit_test_blocker_operations, resolve_windows_panel_paint_operations,
        WindowsPanelPaintColor, WindowsPanelPaintOperation,
    },
};
...
pub(super) trait WindowsPanelPainter {
    fn paint(&mut self, job: &WindowsPanelShellPaintJob) -> Result<WindowsPanelPaintPlan, String>;
}

#[derive(Default)]
struct NativePanelPlanSubmissionRecorder;

impl reef::draw::draw_backend::DrawBackend for NativePanelPlanSubmissionRecorder {
    type Error = String;
...
impl WindowsDirect2DCoordinateSpace {
    ...
    pub(super) fn rect(self, rect: Rect) -> PanelRect {
        PanelRect {
            x: rect.x,
    ...
    pub(super) fn point(self, point: Point) -> PanelPoint {
        PanelPoint {
            x: point.x,
...
pub(super) struct Direct2DWindowsPanelPainter {
...
impl Direct2DWindowsPanelPainter {
...
impl WindowsPanelPainter for Direct2DWindowsPanelPainter {
    fn paint(&mut self, job: &WindowsPanelShellPaintJob) -> Result<WindowsPanelPaintPlan, String> {
...
impl Direct2DWindowsPanelPainter {
    fn paint_plan_to_layered_window(
        ...
        unsafe {
            ...
            let mut operations = resolve_windows_panel_hit_test_blocker_operations(job);
            operations.extend(resolve_windows_panel_paint_operations(plan));
            for operation in operations {
                match operation {
                    WindowsPanelPaintOperation::PushClip { frame } => {
                        surface.target.PushAxisAlignedClip(
                            &d2d_rect(coordinate_space.rect(frame)),
                            D2D1_ANTIALIAS_MODE_PER_PRIMITIVE,
                        );
                    }
                    WindowsPanelPaintOperation::PopClip => {
                        surface.target.PopAxisAlignedClip();
                    }
                    WindowsPanelPaintOperation::DrawImage { .. } => {}
                    WindowsPanelPaintOperation::DrawNineSliceImage { frame, opacity, .. } => {
                    ...
                    WindowsPanelPaintOperation::DrawSpriteImage {
                        ...
                        draw_mascot_sprite_image(
                            ...
                            bitmap,
                            coordinate_space,
                            panel_rect_from_rect(source_rect),
                            panel_rect_from_rect(frame),
                            opacity,
                        );
                    }
                    ...
                    WindowsPanelPaintOperation::DrawText {
                        ...
                        let brush = surface
                            ...
                            .CreateSolidColorBrush(&d2d_color_with_alpha(color, alpha), None)
                            .map_err(|error| error.to_string())?;
                        let text_rect = d2d_rect(coordinate_space.rect(frame));
                    ...
                    WindowsPanelPaintOperation::FillStrokedRoundRect {
                        ...
                        let fill_brush = surface
                            .target
                            .CreateSolidColorBrush(&d2d_color_with_alpha(fill, alpha), None)
                            .map_err(|error| error.to_string())?;
                    ...
                    WindowsPanelPaintOperation::FillPath {
                        ...
                        let brush = surface
                            ...
                            .CreateSolidColorBrush(&d2d_color_with_alpha(fill, alpha), None)
                            .map_err(|error| error.to_string())?;
                        let geometry = d2d_path_geometry(
                            self.direct2d
                                .factory()
...
fn draw_completion_glow_image(
    ...
    for slice in crate::presentation::presentation::resolve_completion_glow_image_slices(frame) {
        let dest = slice.dest;
        let source = slice.source;
        if dest.width <= 0.0 || dest.height <= 0.0 || source.width <= 0.0 || source.height <= 0.0 {
            continue;
        }
        let dest = d2d_rect(coordinate_space.rect(rect_from_panel_rect(dest)));
...
fn draw_mascot_sprite_image(
    ...
    let dest = d2d_rect(coordinate_space.rect(rect_from_panel_rect(frame)));
...
#[cfg(all(windows, not(test)))]
fn d2d_path_geometry(
    factory: &windows::Win32::Graphics::Direct2D::ID2D1Factory,
    coordinate_space: WindowsDirect2DCoordinateSpace,
    segments: &[reef::draw::primitive::PathSegment],
    ...
    let Some(first) = segments.first() else {
        return unsafe { factory.CreatePathGeometry() }.map_err(|error| error.to_string());
    };
    let start = match first {
        reef::draw::primitive::PathSegment::LineTo(point) => *point,
        reef::draw::primitive::PathSegment::CubicBezier { end, .. } => *end,
    };
    ...
    unsafe {
        sink.BeginFigure(
            d2d_point(coordinate_space.point(start)),
            D2D1_FIGURE_BEGIN_FILLED,
        );
        for segment in segments.iter().skip(1) {
            match segment {
                reef::draw::primitive::PathSegment::LineTo(point) => {
                    sink.AddLine(d2d_point(coordinate_space.point(*point)));
                }
                reef::draw::primitive::PathSegment::CubicBezier {
                    control1,
                    control2,
                    end,
                } => {
                    sink.AddBezier(&D2D1_BEZIER_SEGMENT {
                        point1: d2d_point(coordinate_space.point(*control1)),
                        point2: d2d_point(coordinate_space.point(*control2)),
                        point3: d2d_point(coordinate_space.point(*end)),
                    });
                }
            }
        }
...
fn panel_rect_from_rect(rect: Rect) -> PanelRect {
    PanelRect {
        x: rect.x,
        y: rect.y,
        width: rect.width,
...
fn rect_from_panel_rect(rect: PanelRect) -> Rect {
    Rect {
        x: rect.x,
        y: rect.y,
        width: rect.width,
...
#[derive(Debug, Default)]
pub(super) struct PlanOnlyWindowsPanelPainter;

impl WindowsPanelPainter for PlanOnlyWindowsPanelPainter {
    fn paint(&mut self, job: &WindowsPanelShellPaintJob) -> Result<WindowsPanelPaintPlan, String> {
        Ok(resolve_windows_panel_paint_plan(job))
    }
}

#[cfg(all(windows, not(test)))]
#[derive(Debug)]
pub(super) struct GdiWindowsPanelPainter {
    raw_window_handle: Option<isize>,
}

#[cfg(all(windows, not(test)))]
impl GdiWindowsPanelPainter {
...
#[cfg(all(windows, not(test)))]
impl WindowsPanelPainter for GdiWindowsPanelPainter {
    fn paint(&mut self, job: &WindowsPanelShellPaintJob) -> Result<WindowsPanelPaintPlan, String> {
        super::paint_backend::paint_windows_panel_job_with_gdi(self.raw_window_handle, job)
    }
}
