...
fn scene_builder_sanitizes_completion_message_for_compact_headline() {
    ...
    completion.last_assistant_message =
        Some("apps/desktop/src-tauri/src/windows_panel/host_runtime.rs\nsrc/runtime".to_string());
    let state = PanelState {
        status_queue: vec![StatusQueueItem {
            key: "completion:session-1".to_string(),
            session_id: "session-1".to_string(),
...
fn scene_builder_emits_settings_rows_and_value_badges() {
    ...
    let input = PanelSceneBuildInput {
        display_options: vec![
            crate::scene::panel_display_option_state(0, "display-1", "Built-in", 3024, 1964),
            crate::scene::panel_display_option_state(1, "display-2", "Studio Display", 2560, 1440),
            crate::scene::panel_display_option_state(2, "display-3", "Projector", 1920, 1080),
        ],
    ...
    assert_eq!(
        ...
        crate::scene::SettingsSurfaceControlKind::Action
    );
...
fn settings_scene_hides_wide_width_on_notch_display() {
    ...
    let input = PanelSceneBuildInput {
        display_options: vec![crate::scene::panel_display_option_state_with_width_support(
            0,
            "display-1",
            "Built-in",
            3024,
        ...
        settings: PanelSettingsState {
            ...
            island_width_preset: crate::state::PanelIslandWidthPreset::Wide,
...
fn settings_scene_localizes_panel_language_to_chinese() {
    let scene = build_panel_scene(
        ...
        &PanelSceneBuildInput {
            settings: PanelSettingsState {
                language: crate::state::PanelLanguage::Zh,
