                ...
                loop {
                    ...
                    if input.peek(Token![<]) && input.peek2(Token![/]) {
                        ...
                        let close_name: Ident = input.parse().map_err(|_| {
                            input.error(format!(
                                "expected tag name after `</` to close `<{}>`",
                                tag_name
                            ))
                        })?;
...
fn gen_prop_value(value: &RsxAttrValue) -> TokenStream2 {
    match value {
        RsxAttrValue::Str(s) => {
            quote! { ::reef::vnode::PropValue::String(#s.to_string()) }
        }
        RsxAttrValue::Expr(tokens) => {
            quote! { ::reef::vnode::PropValue::from(#tokens) }
        }
        RsxAttrValue::Bool(b) => {
            quote! { ::reef::vnode::PropValue::Bool(#b) }
        }
    }
}
...
fn gen_node(node: &RsxNode) -> TokenStream2 {
    match node {
        RsxNode::Element {
            ...
            if is_component {
                ...
                if child_code.is_empty() {
                    ...
                    if attrs.is_empty() {
                        quote! {
                            ::reef::vnode::VNode::VElement(::reef::vnode::VElement {
                                ty: ::reef::vnode::ElementType::Function(#name_ident),
                                props: ::reef::vnode::PropsMap::new(),
                                children: ::std::vec![],
                        ...
                        quote! {
                            {
                                let mut __props = ::reef::vnode::PropsMap::new();
                                #(#attr_stmts)*
                                ::reef::vnode::VNode::VElement(::reef::vnode::VElement {
                                    ty: ::reef::vnode::ElementType::Function(#name_ident),
                                    props: __props,
                    ...
                    if attrs.is_empty() {
                        quote! {
                            {
                                let mut __props = ::reef::vnode::PropsMap::new();
                                __props.insert("__children", ::reef::vnode::PropValue::VNodeList(::std::vec![#(#child_code),*]));
                                ::reef::vnode::VNode::VElement(::reef::vnode::VElement {
                                    ty: ::reef::vnode::ElementType::Function(#name_ident),
                                    props: __props,
                                    children: ::std::vec![],
                ...
                if attrs.is_empty() {
                    quote! {
                        ::reef::vnode::element(#name, ::reef::vnode::PropsMap::new(), vec![#(#child_code),*])
                    }
                } else {
                    ...
                    quote! {
                        {
                            let mut __props = ::reef::vnode::PropsMap::new();
                            #(#attr_stmts)*
                            ::reef::vnode::element(#name, __props, vec![#(#child_code),*])
                        }
                    }
                }
            }
        ...
        RsxNode::Text(text) => {
            quote! { ::reef::vnode::VNode::VText(#text.to_string()) }
        }
        RsxNode::Expr(tokens) => {
            quote! { #tokens }
        }
...
fn gen_root(root: &RsxRoot) -> TokenStream2 {
    ...
    if nodes.len() == 1 {
        ...
        quote! { ::reef::vnode::VNode::VFragment(vec![#(#nodes),*]) }
    }
}
...
impl Parse for PropsMacroInput {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        while !input.is_empty() {
            ...
            pairs.push(PropPair { key, _colon, value });
