...
impl From<f64> for PropValue {
    fn from(v: f64) -> Self { PropValue::F64(v) }
}

impl From<i32> for PropValue {
    fn from(v: i32) -> Self { PropValue::I32(v) }
}
...
impl Clone for PropsMap {
    fn clone(&self) -> Self {
        Self { entries: self.entries.iter().map(|(k, v)| (*k, v.clone())).collect() }
    }
}
...
impl PartialEq for PropsMap {
    fn eq(&self, other: &Self) -> bool {
        ...
        self.entries.iter().all(|(k, v)| {
            other.entries.iter().any(|(ok, ov)| k == ok && v == ov)
        })
    }
}
