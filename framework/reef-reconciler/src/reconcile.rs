...
use crate::fiber::{EffectTag, ElementTypeRef, FiberNode};
        ...
        match child_vnode {
            ...
            VNode::VElement(el) => {
                ...
                let old_id = old_map.remove(&el_key).or_else(|| {
                    ...
                    remaining_old.iter().position(|id| {
                        let f = arena.get(*id);
...
mod tests {
    ...
    fn reconcile_single_child_placement() {
        ...
        let child = VNode::VElement(reef_vnode::VElement {
        ...
        reconcile_children(&mut arena, root, vec![child], &ElementTypeRef::Native("root"));
    ...
    fn reconcile_multiple_children() {
        ...
        let children = vec![
            VNode::VElement(reef_vnode::VElement {
                ty: ElementType::Native("label"),
                props: PropsMap::new(),
                children: vec![],
                key: Some("a".into()),
    ...
    fn reconcile_text_children() {
        ...
        assert_eq!(arena.get(child).element_type, ElementTypeRef::Native("#text"));
