use super::{
    directwrite_text_requests_from_paint_plan, Direct2DWindowsNativePanelPainter,
    PlanOnlyWindowsNativePanelPainter, WindowsDirect2DCoordinateSpace, WindowsNativePanelPainter,
};
use crate::{
...
fn paint_job() -> WindowsNativePanelShellPaintJob {
    let zero = PanelRect {
        x: 0.0,
        y: 0.0,
        width: 0.0,
...
#[test]
fn directwrite_requests_use_text_frame_width_and_reef_draw_text_style() {
    let plan = DrawPlan {
        hidden: false,
        viewport: Size {
            width: 100.0,
...
fn direct2d_painter_constructs_without_window() {
    let painter = Direct2DWindowsNativePanelPainter::new(None).expect("painter");
