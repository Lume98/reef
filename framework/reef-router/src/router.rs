use crate::navigator::{HISTORY, HISTORY_INDEX};
use crate::matcher::match_path;
use crate::route::set_current_path;
use reef_vnode::{PropsMap, VElement, VNode};
...
pub fn router(children: Vec<VNode>) -> VNode {
    ...
    VNode::VElement(VElement {
        ty: reef_vnode::ElementType::Native("$context_provider"),
        props: {
...
pub fn route(path: &str, component: fn() -> VNode) -> VNode {
    let current_path = crate::route::use_current_path();
...
pub fn navigate_to(path: &str) {
    use crate::navigator::use_navigator;
    ...
    fn router_creates_context_provider() {
        ...
        match &result {
            VNode::VElement(el) => {
                assert_eq!(
                    el.ty,
                    reef_vnode::ElementType::Native("$context_provider")
                );
